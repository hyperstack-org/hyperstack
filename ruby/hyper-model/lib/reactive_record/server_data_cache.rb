...
=end
    ...
    def self.load_from_json(tree, target = nil)
      `
        try {
          let sorted_collection = tree['$[]']('*all');

          if ($truthy(sorted_collection)) {
            let loaded_collection = [];

            for (let i = 0; i < sorted_collection.length; i++) {
              let id = sorted_collection[i] || Opal.NIL;

              loaded_collection.push(
                Opal.ReactiveRecord.Base.$find_by_id(target.$proxy_association().$klass(), id)
              );
            };

            if ( $truthy(loaded_collection[0]) &&
                 $truthy(target.$scope_description()) &&
                 target.$scope_description() == '___hyperstack_internal_scoped_find_by' ) {
              let primary_key = target.$proxy_association().$klass().$primary_key();

              let attrs = $send(target.$vector()['$[]'](-1)['$[]'](1), 'reject', [], ((key, _) => {
                if (key == null) key = Opal.NIL;if (_ == null) _ = Opal.NIL;
            ...
            $send(Opal.Hyperstack.$const_get('Internal::State::Mapper'), 'bulk_update', [], (() => {
              Opal.Hyperstack.$const_get('Internal::State::Variable').$set(target, 'collection', target.$collection());
            }));
          };

          let id_value = tree['$[]']('id');

          if (id_value && Array.isArray(id_value)) {
            let id = id_value[0] || Opal.NIL;

            target.$send('id=', id)
          };

          for (let i = 0; i < tree.$length(); i ++) {
            let method = tree.$keys()[i] || Opal.NIL,
              value = tree['$[]'](method) || Opal.NIL,
              new_target = Opal.NIL,
              aggregation = Opal.NIL,
              association = Opal.NIL,
              klass = Opal.NIL;

            try { method = Opal.JSON.$parse(method) } catch {};

            if (method == '*all') {
              return Opal.NIL;
            } else if ($truthy(target['$!']())) {
              self.$load_from_json(value, Opal.Object.$const_get(method));
            } else if (method == '*count') {
              target.$set_count_state(value[0]);
            } else if (method['$is_a?'](Opal.Integer) || /^[0-9]+$/.test(method)) {
              new_target = target.$push_and_update_belongs_to(method);
            } else if (Array.isArray(method)) {
              if (method[0] == 'new') {
                new_target = Opal.ReactiveRecord.Base.$lookup_by_object_id(method[1]);
              } else if (!$truthy(target.$class()['$<'](Opal.ReactiveRecord.Base))) {
                new_target = target.$send(...Opal.to_a(method));

                if (Array.isArray(value) && $truthy(new_target.$destroy())) {
                  new_target = Opal.NIL;
                };
              } else {
                target.$backing_record().$update_simple_attribute([method], target.$backing_record().$convert(method, value[0]))
              }
            } else if ( target.$class()['$respond_to?']('reflect_on_aggregation') &&
                        $truthy(aggregation = target.$class().$reflect_on_aggregation(method)) &&
                        !$truthy(aggregation.$klass()['$<'](Opal.ActiveRecord.Base)) ) {
              if (!(value[0]['$is_a?'](aggregation.$klass()))) {
                value = [aggregation.$deserialize(value[0])]
              };

              target.$send("" + (method) + "=", value[0]);
            } else if (Array.isArray(value)) {
              if (method != 'id') {
                target.$backing_record().$set_attr_value(method, value[0])
              }
            } else if ( value['$is_a?'](Opal.Hash) &&
                        $truthy(value['$[]']('id')) &&
                        $truthy(value['$[]']('id')[0]) &&
                        $truthy(association = target.$class().$reflect_on_association(method))) {
              if ($truthy(value['$[]']('model_name'))) {
                klass = Object.$const_get(value['$[]']("model_name")[0]);
              } else {
                klass = association.$klass();
              };

              new_target = Opal.ReactiveRecord.Base.$find_by_id(klass, value['$[]']('id')[0]);

              target.$send("" + (method) + "=", new_target);
            } else if (!$truthy(target.$class()['$<'](Opal.ActiveRecord.Base))) {
            ...
            if ($truthy(new_target)) {
              self.$load_from_json(value, new_target);
            };
          };
