    ...
    module PubSub
      class << self
        def publish_record(record)
          message = { record.class.to_s.underscore => { instances: { record.id => { properties: record.as_json,
        ...
        def publish_relation(base_record, relation_name, record = nil)
          message = { base_record.class.to_s.underscore => { instances: { base_record.id => { properties: {
        ...
        def publish_rest_method(record, method_name)
          s_method_name, s_method_args = method_name.to_s.split('_[')
          s_method_args = s_method_args ? '[' + s_method_args : '[]'
          message = { record.class.to_s.underscore => { instances: { record.id => { properties: {
            updated_at: record.updated_at,
            destroyed: record.destroyed?
        ...
        def pub_sub_rest_method(session_id, record, rest_method_name)
          subscribe_rest_method(session_id, record, rest_method_name)
