...
module HyperSpec
  module ComponentTestHelpers
    ...
    def evaluate_ruby(str = '', opts = {}, &block)
      ...
      insure_mount
      JSON.parse(evaluate_script("[#{compile_to_ruby(str, &block)}].$to_json()"), opts).first
    end

    # handy for debugging extreme issues where we want to see what the compiler actually generates
    def compile_to_ruby(str = '', &block)
      if block
        str = "#{str}\n#{Unparser.unparse Parser::CurrentRuby.parse(block.source).children.last}"
      end
      Opal.compile(str).gsub("// Prepare super implicit arguments\n", "")
          .delete("\n").gsub('(Opal);', '(Opal)')
    end
