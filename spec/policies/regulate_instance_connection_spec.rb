...
describe "regulate_instance_connections" do
  ...
  it "will fail for an instance connection if the connection policy does not support instances" do
    ...
    expect { Hyperloop::InternalPolicy.regulate_connection(Application.find(123), "Application-123") }.to raise_error('connection failed')
  end
  ...
  it "will succeed for an instance connection if the connection policy does support instances" do
    ...
    expect { Hyperloop::InternalPolicy.regulate_connection(Application.find(123), "Application-123") }.not_to raise_error
  end

  it "will succeed only if the user id is correct" do
    ApplicationPolicy.regulate_instance_connections { Application.find('baz') }
    expect { Hyperloop::InternalPolicy.regulate_connection(nil, "Application-baz") }.not_to raise_error
  end
  ...
  it "can be applied to a different class" do
    ...
    expect { Hyperloop::InternalPolicy.regulate_connection(Application.find('baz'), "Application-baz") }.to raise_error('connection failed')
  end

  it "can return an array objects" do
    ApplicationPolicy.regulate_instance_connections { [Application.find(1), Application.find(2)] }
    expect { Hyperloop::InternalPolicy.regulate_connection("foo", "Application-1") }.not_to raise_error
    expect { Hyperloop::InternalPolicy.regulate_connection("foo", "Application-2") }.not_to raise_error
  end

  it "can conditionally regulate the connection" do
    ApplicationPolicy.regulate_instance_connections { id == '1' && self }
    expect { Hyperloop::InternalPolicy.regulate_connection(Application.new(1), "Application-1") }.not_to raise_error
    expect { Hyperloop::InternalPolicy.regulate_connection(Application.new(2), "Application-2") }.to raise_error('connection failed')
  end
